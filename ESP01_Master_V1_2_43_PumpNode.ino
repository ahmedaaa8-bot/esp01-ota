...
#define FW_VERSION         "1.2.43"
#define FW_BUILD           1243
#define FW_NAME            "Ahmed Monem Smart Control System"
#define FW_DEVICE_NAME     "Weather and Control "
#define FW_FULL_NAME       "Ahmed Control System +201004608852 . V" FW_VERSION " EEPROM Backup Restore + Pump Node UDP By Ahmed Monem "
...
const unsigned long PUMP_NODE_SYNC_INTERVAL_MS = 10000UL; // Periodic re-sync even if state does not change
const unsigned long PUMP_NODE_ACK_TIMEOUT_MS  = 15000UL;  // After this, web shows Pump Node OFFLINE
...
bool pumpNodeIsOnline() {
  ...
  return pumpNodeLastAckMs > 0 && (millis() - pumpNodeLastAckMs) <= PUMP_NODE_ACK_TIMEOUT_MS;
#else
  return false;
#endif
}
...
void pumpNodeSendSyncNow(const char* reason) {
  ...
  char msg[96];
  snprintf(msg, sizeof(msg), "PNCMD,%s,%lu,%u,%lu", PUMP_NODE_SHARED_KEY,
           (unsigned long)pumpNodeLastCommandId,
           (unsigned)currentOutput1State,
           (unsigned long)millis());

  // Send to any node on the router side and also to the Master AP side.
  pumpNodeSendTo(IPAddress(255, 255, 255, 255), msg);
  pumpNodeSendTo(IPAddress(192, 168, 4, 255), msg);
  if (pumpNodeLastIP != IPAddress(0, 0, 0, 0)) pumpNodeSendTo(pumpNodeLastIP, msg);

  pumpNodeLastSendMs = millis();
  pumpNodeSyncRequested = false;
  DBG_PRINT("Pump Node sync: ");
  DBG_PRINT(reason);
  DBG_PRINT(" -> ");
  DBG_PRINTLN(msg);
#endif
...
void pumpNodeHandlePacket() {
  ...
  // Expected ACK: PNACK,KEY,cmdId,state,rssi,mode
  char* p0 = strtok(buf, ",");
  ...
  if (ackId == pumpNodeLastCommandId || ackId > 0) {
    pumpNodeAckState = ackState;
    pumpNodeLastAckMs = millis();
    if (p4) pumpNodeRssi = atoi(p4);
    if (p5) {
      strncpy(pumpNodeMode, p5, sizeof(pumpNodeMode) - 1);
...
void pumpNodeLoop() {
  ...
  if (pumpNodeSyncRequested || (now - pumpNodeLastSendMs >= PUMP_NODE_SYNC_INTERVAL_MS)) {
...
String outputControlCardHtml() {
  ...
  html += F("<div class='row'><span class='k'>Mode / RSSI</span><span class='v'><span id='pnmode'>--</span> / <span id='pnrssi'>--</span></span></div>");
