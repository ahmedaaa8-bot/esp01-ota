...
#define FW_FULL_NAME       "Ahmed Control System +201004608852 . V" FW_VERSION " IMPROVED: Secure Credentials + Strong Sessions + Runtime Hours + Heap Watchdog"
...
// WiFi Credentials — empty. Set via web UI at 192.168.4.1/wifi after first boot.
const char* ssid     = "";
const char* password = "";

// ThingSpeak Settings — empty. Set via web UI at 192.168.4.1 after first boot.
const unsigned long DEFAULT_CHANNEL_ID = 0UL;
const char* DEFAULT_WRITE_API_KEY = "";
const char* DEFAULT_READ_API_KEY  = "";
...
// Runtime Hours Counter EEPROM (V2.0)
// Stores total ON-time seconds for Output1 and Output2. Safe area: 1564-1575.
#define EEPROM_HOURS_MAGIC_ADDR  1564
#define EEPROM_HOURS_OUT1_ADDR   1566   // 4 bytes uint32_t
#define EEPROM_HOURS_OUT2_ADDR   1570   // 4 bytes uint32_t
#define EEPROM_HOURS_MAGIC       0xB3
...
const char* DEFAULT_DUCKDNS_DOMAIN = "";   // Set via web UI at /wifi page
const char* DEFAULT_DUCKDNS_TOKEN  = "";   // Set via web UI at /wifi page
...
// Runtime hours tracking (V2.0)
uint32_t output1TotalSec = 0;
uint32_t output2TotalSec = 0;
unsigned long output1OnStartMs = 0;
unsigned long output2OnStartMs = 0;
const unsigned long HOURS_SAVE_INTERVAL_MS = 60000UL;
unsigned long lastHoursSaveMs = 0;

// Login rate limiting (V2.0)
byte loginFailCount = 0;
unsigned long loginLockUntilMs = 0;
...
String makeSessionToken() {
  // esp_random() uses the hardware RNG of ESP32 — 128-bit entropy, not predictable.
  char buf[33];
...
void handleDoLogin() {
  // Rate limiting: lock for 5 minutes after 5 failed attempts
  if (loginFailCount >= 5 && millis() < loginLockUntilMs) {
    webServer.sendHeader("Location", "/login?bad=1", true);
    webServer.send(302, "text/plain", "");
    return;
  }
  if (millis() >= loginLockUntilMs && loginLockUntilMs != 0) {
    loginFailCount = 0;
    loginLockUntilMs = 0;
  }
  ...
  loginFailCount++;
  if (loginFailCount >= 5) {
    loginLockUntilMs = millis() + 300000UL; // 5 minutes lock
    DBG_PRINTLN("Login locked 5 min after 5 failed attempts");
  }
  webServer.sendHeader("Location", "/login?bad=1", true);
  webServer.send(302, "text/plain", "");
}
...
String outputControlCardHtml() {
  ...
  html += F("<div class='ocbox'><div class='octop'><div><b>PUMP</b><div class='small'>Output 1 / GPIO26</div></div><div><span id='o1' class='bigstate "); html += outputClass(currentOutput1State); html += F("'>"); html += outputStateText(currentOutput1State); html += F("</span><span id='d1' class='dot "); html += outputClass(currentOutput1State); html += F("'></span></div></div>");
  html += F("<div class='small' style='color:#94a3b8;margin:4px 0 6px'>Runtime: <span id='rt1'>"); html += formatRuntimeOut1(); html += F("</span></div>");
  html += F("<button id='b1' class='btn' onclick='toggleOut(1)'>...</button></div>");

  html += F("<div class='ocbox'><div class='octop'><div><b>LIGHT</b><div class='small'>Output 2 / GPIO27</div></div><div><span id='o2' class='bigstate "); html += outputClass(currentOutput2State); html += F("'>"); html += outputStateText(currentOutput2State); html += F("</span><span id='d2' class='dot "); html += outputClass(currentOutput2State); html += F("'></span></div></div>");
  html += F("<div class='small' style='color:#94a3b8;margin:4px 0 6px'>Runtime: <span id='rt2'>"); html += formatRuntimeOut2(); html += F("</span></div>");
...
String outputStatusJson(bool changed = false) {
  ...
  json += F("\"");
  json += F(",\"rt1\":\"");
  json += formatRuntimeOut1();
  json += F("\"");
  json += F(",\"rt2\":\"");
  json += formatRuntimeOut2();
  json += F("\"");
  json += F("}");
  return json;
}
...
bool setOutputFromWeb(byte outputNumber, byte newState) {
  ...
  if (outputNumber == 1) {
    if (currentOutput1State != newState) {
      // Track runtime: save elapsed seconds when turning OFF
      if (newState == 0 && output1OnStartMs > 0) {
        output1TotalSec += (uint32_t)((millis() - output1OnStartMs) / 1000UL);
        output1OnStartMs = 0;
        saveRuntimeHours();
      }
      // Track runtime: record start time when turning ON
      if (newState == 1) output1OnStartMs = millis();
    ...
    if (currentOutput2State != newState) {
      if (newState == 0 && output2OnStartMs > 0) {
        output2TotalSec += (uint32_t)((millis() - output2OnStartMs) / 1000UL);
        output2OnStartMs = 0;
        saveRuntimeHours();
      }
      if (newState == 1) output2OnStartMs = millis();
...
void sendDashboardPage() {
  ...
  html += F("<div class='card'><b>Runtime Hours</b><div class='row'><span class='k'>PUMP</span><span class='v' id='rt1'>"); html += formatRuntimeOut1(); html += F("</span></div><div class='row'><span class='k'>LIGHT</span><span class='v' id='rt2'>"); html += formatRuntimeOut2(); html += F("</span></div><form method='POST' action='/resethours' onsubmit=\"return confirm('Reset runtime hours for all outputs?');\">"); html += sessionHiddenInput(); html += F("<button class='btn btn2' type='submit' style='margin-top:8px'>Reset Runtime Hours</button></form></div>");
  html += F("<script>");
...
webServer.collectHeaders(headerkeys, 1);
  ...
  webServer.on("/resethours", HTTP_POST, handleResetHours);
...
String formatRuntimeOut1() {
  uint32_t live = output1TotalSec;
  if (currentOutput1State && output1OnStartMs > 0)
    live += (uint32_t)((millis() - output1OnStartMs) / 1000UL);
  char buf[16];
  snprintf(buf, sizeof(buf), "%luh %02lum", (unsigned long)(live / 3600UL), (unsigned long)((live % 3600UL) / 60UL));
  return String(buf);
}

String formatRuntimeOut2() {
  uint32_t live = output2TotalSec;
  if (currentOutput2State && output2OnStartMs > 0)
    live += (uint32_t)((millis() - output2OnStartMs) / 1000UL);
  char buf[16];
  snprintf(buf, sizeof(buf), "%luh %02lum", (unsigned long)(live / 3600UL), (unsigned long)((live % 3600UL) / 60UL));
  return String(buf);
}

void loadRuntimeHours() {
  if (EEPROM.read(EEPROM_HOURS_MAGIC_ADDR) == EEPROM_HOURS_MAGIC) {
    EEPROM.get(EEPROM_HOURS_OUT1_ADDR, output1TotalSec);
    EEPROM.get(EEPROM_HOURS_OUT2_ADDR, output2TotalSec);
  } else {
    output1TotalSec = 0;
    output2TotalSec = 0;
  }
  // If output was ON before reset, start counting from now
  if (currentOutput1State) output1OnStartMs = millis();
  if (currentOutput2State) output2OnStartMs = millis();
}

void saveRuntimeHours() {
  EEPROM.write(EEPROM_HOURS_MAGIC_ADDR, EEPROM_HOURS_MAGIC);
  EEPROM.put(EEPROM_HOURS_OUT1_ADDR, output1TotalSec);
  EEPROM.put(EEPROM_HOURS_OUT2_ADDR, output2TotalSec);
  EEPROM.commit();
}

void resetRuntimeHours() {
  output1TotalSec = 0;
  output2TotalSec = 0;
  output1OnStartMs = currentOutput1State ? millis() : 0;
  output2OnStartMs = currentOutput2State ? millis() : 0;
  saveRuntimeHours();
}

void updateRuntimeHours() {
  unsigned long now = millis();
  if (currentOutput1State && output1OnStartMs > 0) {
    output1TotalSec += (uint32_t)((now - output1OnStartMs) / 1000UL);
    output1OnStartMs = now;
  }
  if (currentOutput2State && output2OnStartMs > 0) {
    output2TotalSec += (uint32_t)((now - output2OnStartMs) / 1000UL);
    output2OnStartMs = now;
  }
  if (now - lastHoursSaveMs >= HOURS_SAVE_INTERVAL_MS) {
    lastHoursSaveMs = now;
    saveRuntimeHours();
  }
}
...
void handleResetHours() {
  ...
  webServer.send(303, "text/plain", "Reset");
}
...
void loop() {
  ...
  // Heap watchdog: restart if memory critically low (V2.0)
  if (ESP.getFreeHeap() < 8000) {
    DBG_PRINTLN("CRITICAL: heap too low, saving hours and restarting");
    saveRuntimeHours();
    delay(200);
    ESP.restart();
  }
