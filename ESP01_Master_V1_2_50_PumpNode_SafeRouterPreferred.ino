...
#define FW_VERSION         "1.2.50"
#define FW_BUILD           1248
#define FW_NAME            "Ahmed Monem Smart Control System"
#define FW_DEVICE_NAME     "Weather and Control "
#define FW_FULL_NAME       "Ahmed Control System +201004608852 . V" FW_VERSION " EEPROM Backup Restore + Pump Node SafeRouterPreferred KeepState UDP By Ahmed Monem "
...
// Pump Node UDP Mirror / Monitor (V1.2.43)
// Master remains the ONLY decision maker. Output1/PUMP relay on this master is untouched.
// The Pump Node only mirrors the final pump state and returns ACK/status.
#define ENABLE_PUMP_NODE_UDP 1
const uint16_t PUMP_NODE_UDP_PORT = 4210;                 // Same port on master and node
const unsigned long PUMP_NODE_SYNC_INTERVAL_MS = 10000UL; // Periodic reply/sync even if state does not change
const unsigned long PUMP_NODE_ACK_TIMEOUT_MS  = 60000UL;  // HELLO-based timeout. Node keeps last pump state if network is lost.
const unsigned long PUMP_NODE_BOOT_SYNC_DELAY_MS = 5000UL;
const char* PUMP_NODE_SHARED_KEY = "PN1234";              // Must match the Pump Node sketch
...
#define EEPROM_BACKUP_MAP_VERSION 1241
...
WiFiClient client;
WiFiUDP pumpNodeUdp;
bool pumpNodeUdpStarted = false;
uint32_t pumpNodeLastCommandId = 0;
byte pumpNodeLastCommandState = 0;
byte pumpNodeAckState = 0;
int pumpNodeRssi = 0;
char pumpNodeMode[16] = "--";
bool pumpNodeRouterSeen = false;
unsigned long pumpNodeLastSwitchAge = 0;
IPAddress pumpNodeLastIP;
unsigned long pumpNodeLastSendMs = 0;
unsigned long pumpNodeLastAckMs = 0;
unsigned long pumpNodeLastRxMs = 0;
bool pumpNodeSyncRequested = true;
ESP8266WebServer* webServerPtr = nullptr;
...
// ========================= PUMP NODE UDP MIRROR =========================
// V1.2.46 stable design:
// - Pump Node sends PNHELLO directly to the master.
// - In AP mode it sends to 192.168.4.1. In router mode it can send to broadcast or fixed master IP.
// - Master marks node online from HELLO, not only from ACK.
// - Master replies directly to the sender IP with current pump state.

bool pumpNodeIsOnline() {
#if ENABLE_PUMP_NODE_UDP
  return pumpNodeLastRxMs > 0 && (millis() - pumpNodeLastRxMs) <= PUMP_NODE_ACK_TIMEOUT_MS;
#else
  return false;
#endif
}

String pumpNodeIpText() {
#if ENABLE_PUMP_NODE_UDP
  if (pumpNodeLastIP == IPAddress(0, 0, 0, 0)) return F("--");
  return pumpNodeLastIP.toString();
#else
  return F("disabled");
#endif
}
...
void pumpNodeBegin() {
#if ENABLE_PUMP_NODE_UDP
  if (!pumpNodeUdpStarted) {
    pumpNodeUdpStarted = pumpNodeUdp.begin(PUMP_NODE_UDP_PORT);
    DBG_PRINT("Pump Node UDP begin on port ");
    DBG_PRINT(PUMP_NODE_UDP_PORT);
    DBG_PRINT(" -> ");
    DBG_PRINTLN(pumpNodeUdpStarted ? "OK" : "FAILED");
  }
  pumpNodeSyncRequested = true;
#endif
}

void pumpNodeRequestSync() {
#if ENABLE_PUMP_NODE_UDP
  pumpNodeSyncRequested = true;
#endif
}

void pumpNodeSendTo(IPAddress ip, const char* payload) {
#if ENABLE_PUMP_NODE_UDP
  if (!pumpNodeUdpStarted) return;
  pumpNodeUdp.beginPacket(ip, PUMP_NODE_UDP_PORT);
  pumpNodeUdp.write((const uint8_t*)payload, strlen(payload));
  pumpNodeUdp.endPacket();
#endif
}

void pumpNodeBuildCommand(char* msg, size_t msgSize) {
#if ENABLE_PUMP_NODE_UDP
  pumpNodeLastCommandId++;
  if (pumpNodeLastCommandId == 0) pumpNodeLastCommandId = 1;
  pumpNodeLastCommandState = currentOutput1State;

  // Extra optional field: master STA IP.
  // The Pump Node learns this IP while connected to Master AP, then can return through Router directly.
  String staIp = (WiFi.status() == WL_CONNECTED) ? WiFi.localIP().toString() : String("0.0.0.0");
  snprintf(msg, msgSize, "PNCMD,%s,%lu,%u,%lu,%s", PUMP_NODE_SHARED_KEY,
           (unsigned long)pumpNodeLastCommandId,
           (unsigned)currentOutput1State,
           (unsigned long)millis(),
           staIp.c_str());
#endif
}

void pumpNodeSendCommandTo(IPAddress ip, const char* reason) {
#if ENABLE_PUMP_NODE_UDP
  if (!pumpNodeUdpStarted) return;
  char msg[96];
  pumpNodeBuildCommand(msg, sizeof(msg));
  pumpNodeSendTo(ip, msg);
  pumpNodeLastSendMs = millis();
  pumpNodeSyncRequested = false;
  DBG_PRINT("Pump Node direct sync: ");
  DBG_PRINT(reason);
  DBG_PRINT(" -> ");
  DBG_PRINT(ip);
  DBG_PRINT(" ");
...
void pumpNodeSendSyncNow(const char* reason) {
#if ENABLE_PUMP_NODE_UDP
  if (!pumpNodeUdpStarted) return;

  // Prefer direct reply to the last known node IP. Broadcast is only fallback.
  if (pumpNodeLastIP != IPAddress(0, 0, 0, 0)) {
    pumpNodeSendCommandTo(pumpNodeLastIP, reason);
    return;
  }

  char msg[96];
  pumpNodeBuildCommand(msg, sizeof(msg));
  pumpNodeSendTo(IPAddress(192, 168, 4, 255), msg);
  pumpNodeSendTo(IPAddress(255, 255, 255, 255), msg);
  pumpNodeLastSendMs = millis();
  pumpNodeSyncRequested = false;

  DBG_PRINT("Pump Node fallback broadcast: ");
  DBG_PRINT(reason);
  DBG_PRINT(" -> ");
  DBG_PRINTLN(msg);
#endif
...
void pumpNodeUpdateFromStatus(byte state, int rssi, const char* modeText, bool routerSeen, unsigned long nodeUptimeSec) {
#if ENABLE_PUMP_NODE_UDP
  if (!isValidState(state)) return;
  pumpNodeAckState = state;
  pumpNodeRssi = rssi;
  pumpNodeRouterSeen = routerSeen;
  pumpNodeLastSwitchAge = nodeUptimeSec;
  pumpNodeLastRxMs = millis();
  pumpNodeLastAckMs = pumpNodeLastRxMs; // Web compatibility: Last ACK means last node contact.
  if (modeText && modeText[0]) {
    strncpy(pumpNodeMode, modeText, sizeof(pumpNodeMode) - 1);
    pumpNodeMode[sizeof(pumpNodeMode) - 1] = '\0';
  }
#endif
}

void pumpNodeHandlePacket() {
#if ENABLE_PUMP_NODE_UDP
  int packetSize = pumpNodeUdp.parsePacket();
  if (packetSize <= 0) return;

  char buf[128];
  int len = pumpNodeUdp.read(buf, sizeof(buf) - 1);
  if (len <= 0) return;
  buf[len] = '\0';

  char original[128];
  strncpy(original, buf, sizeof(original) - 1);
  ...
  char* p7 = strtok(nullptr, ",");
  if (!p0 || !p1) return;
  if (strcmp(p1, PUMP_NODE_SHARED_KEY) != 0) return;

  pumpNodeLastIP = pumpNodeUdp.remoteIP();

  // New stable heartbeat: PNHELLO,KEY,state,rssi,mode,uptime,routerSeen,switchAge
  if (strcmp(p0, "PNHELLO") == 0) {
    byte nodeState = p2 ? (byte)atoi(p2) : 0;
    int rssi = p3 ? atoi(p3) : 0;
    const char* modeText = p4 ? p4 : "--";
    unsigned long switchAgeSec = p7 ? strtoul(p7, nullptr, 10) : (p5 ? strtoul(p5, nullptr, 10) : 0);
    pumpNodeUpdateFromStatus(nodeState, rssi, modeText, p6 ? (atoi(p6) != 0) : false, switchAgeSec);

    // Always answer HELLO directly with the current master pump command.
    pumpNodeSendCommandTo(pumpNodeLastIP, "hello reply");
    return;
  }

  // Compatibility ACK: PNACK,KEY,cmdId,state,rssi,mode
  if (strcmp(p0, "PNACK") == 0) {
    if (!p2 || !p3) return;
    uint32_t ackId = strtoul(p2, nullptr, 10);
    byte ackState = (byte)atoi(p3);
    if (!isValidState(ackState)) return;
    if (ackId == pumpNodeLastCommandId || ackId > 0) {
      pumpNodeUpdateFromStatus(ackState, p4 ? atoi(p4) : 0, p5 ? p5 : "--", pumpNodeRouterSeen, pumpNodeLastSwitchAge);
    }
    return;
  }
#endif
...
void pumpNodeLoop() {
#if ENABLE_PUMP_NODE_UDP
  if (!pumpNodeUdpStarted) pumpNodeBegin();
  pumpNodeHandlePacket();

  unsigned long now = millis();
  if (now < PUMP_NODE_BOOT_SYNC_DELAY_MS) return;

  // Periodic direct re-sync only after the node introduced itself.
  if (pumpNodeSyncRequested || (pumpNodeLastIP != IPAddress(0, 0, 0, 0) && (now - pumpNodeLastSendMs >= PUMP_NODE_SYNC_INTERVAL_MS))) {
    pumpNodeSendSyncNow(pumpNodeSyncRequested ? "requested" : "periodic");
  }
#endif
}
...
String makeSessionToken() {
  String t = String(ESP.getChipId(), HEX);
  t += String(micros(), HEX);
  t += String(millis(), HEX);
...
#if ENABLE_PUMP_NODE_UDP
  html += F("<hr><h2>Pump Node</h2>");
  html += F("<div class='row'><span class='k'>Node Link</span><span class='v'><span id='pnlink' class='pill warn'>--</span></span></div>");
  html += F("<div class='row'><span class='k'>Node Pump</span><span class='v'><span id='pnstate'>--</span></span></div>");
  html += F("<div class='row'><span class='k'>Node IP</span><span class='v'><span id='pnip'>--</span></span></div>");
  html += F("<div class='row'><span class='k'>Current Link / RSSI</span><span class='v'><span id='pnmode'>--</span> / <span id='pnrssi'>--</span> dBm</span></div><div class='row'><span class='k'>Router Status</span><span class='v'><span id='pnrouter'>--</span></span></div><div class='row'><span class='k'>Fail Safe</span><span class='v'>Keep Last Pump State</span></div><div class='row'><span class='k'>Router Logic</span><span class='v'>Safe Preferred / AP Fallback</span></div>");
  html += F("<div class='row'><span class='k'>Last ACK</span><span class='v'><span id='pnack'>--</span> sec</span></div>");
  html += F("<button class='btn btn2' onclick='pumpNodeSync()'>Test / Force Sync Pump Node</button>");
#endif
...
String outputControlJs() {
  ...
  js += F("function paintOut(d){paintOne(1,d.o1);paintOne(2,d.o2);let p=document.getElementById('pend');if(p){p.innerHTML=d.pending?'PENDING':'OK';p.className='pill '+(d.pending?'warn':'ok')}let t=document.getElementById('temp'),h=document.getElementById('hum'),dt=document.getElementById('dhttype');if(t)t.innerHTML=(d.temp===null||d.temp===undefined)?'--':d.temp;if(h)h.innerHTML=(d.hum===null||d.hum===undefined)?'--':d.hum;if(dt)dt.innerHTML=d.dht||'';let o3=document.getElementById('o3');if(o3){o3.innerHTML=d.o3?'ON':'OFF';o3.className=d.o3?'on':'off'}let ns=document.getElementById('ntpstate');if(ns)ns.innerHTML=d.clockState||'No Time';let dv=document.getElementById('devtime');if(dv)dv.innerHTML=d.deviceTime||'Waiting for NTP';let dd=document.getElementById('devday');if(dd)dd.innerHTML=d.deviceDay||'--';let tr=document.getElementById('tremain');if(tr)tr.innerHTML=d.timerRemain||'00:00:00';let oc=document.getElementById('owcity');if(oc)oc.innerHTML=d.city||'--';let ot=document.getElementById('owtemp');if(ot)ot.innerHTML=(d.owTemp===null||d.owTemp===undefined)?'--':d.owTemp;let oh=document.getElementById('owhum');if(oh)oh.innerHTML=(d.owHum===null||d.owHum===undefined)?'--':d.owHum;let os=document.getElementById('owstatus');if(os)os.innerHTML=d.owStatus||'--';let ou=document.getElementById('owtime');if(ou)ou.innerHTML=d.owTime||'--';let pl=document.getElementById('pnlink');if(pl){pl.innerHTML=d.pnOnline?'CONNECTED':'OFFLINE';pl.className='pill '+(d.pnOnline?'ok':'warn')}let ps=document.getElementById('pnstate');if(ps)ps.innerHTML=d.pnState?'ON':'OFF';let pi=document.getElementById('pnip');if(pi)pi.innerHTML=d.pnIP||'--';let pm=document.getElementById('pnmode');if(pm)pm.innerHTML=d.pnMode||'--';let pr=document.getElementById('pnrssi');if(pr)pr.innerHTML=(d.pnRSSI===undefined||d.pnRSSI===0?'--':d.pnRSSI);let prt=document.getElementById('pnrouter');if(prt)prt.innerHTML=d.pnRouterSeen?'Available/Seen':'Not seen or failed';let pa=document.getElementById('pnack');if(pa)pa.innerHTML=(d.pnAckAge===undefined?'--':d.pnAckAge)}");
  ...
  js += F("function pumpNodeSync(){let m=document.getElementById('outmsg');if(m)m.innerHTML='Testing Pump Node...';fetch('/pumpnode/sync?sid="); js += webSessionToken; js += F("').then(r=>{if(!r.ok)throw new Error('auth');return r.json()}).then(d=>{paintOut(d);if(m)m.innerHTML='Pump Node sync command sent';}).catch(e=>{if(m)m.innerHTML='<span class=bad>Pump Node sync failed/session</span>'})}");
...
String outputStatusJson(bool changed = false) {
  ...
  json += F("true");
  json += F(",\"pnOnline\":");
  json += pumpNodeIsOnline() ? F("true") : F("false");
  json += F(",\"pnState\":");
  json += pumpNodeAckState ? F("1") : F("0");
  json += F(",\"pnCmdState\":");
  json += pumpNodeLastCommandState ? F("1") : F("0");
  json += F(",\"pnCmdId\":");
  json += String(pumpNodeLastCommandId);
  json += F(",\"pnRSSI\":");
  json += String(pumpNodeRssi);
  json += F(",\"pnMode\":\"");
  json += pumpNodeMode;
  json += F("\",\"pnIP\":\"");
  json += pumpNodeIpText();
  json += F("\",\"pnRouterSeen\":");
  json += pumpNodeRouterSeen ? F("true") : F("false");
  json += F(",\"pnNodeUptime\":");
  json += String(pumpNodeLastSwitchAge);
  json += F(",\"pnAckAge\":");
  if (pumpNodeLastAckMs) json += String((millis() - pumpNodeLastAckMs) / 1000UL); else json += F("9999");
#else
  json += F("false");
#endif
  json += F(",\"role\":\"");
...
void handlePumpNodeSyncNow() {
#if ENABLE_PUMP_NODE_UDP
  if (!requireRole(WEB_ROLE_OPERATOR)) return;
  pumpNodeRequestSync();
  pumpNodeSendSyncNow("web test/sync");
  webServer.send(200, "application/json", outputStatusJson(false));
#else
  webServer.send(200, "application/json", F("{\"error\":\"disabled\"}"));
#endif
}
...
void setupWebOTA() {
  ...
  if (!webServerPtr) webServerPtr = new ESP8266WebServer(webServerPort);
  ...
  webServer.on("/pumpnode/sync", HTTP_GET, handlePumpNodeSyncNow);
