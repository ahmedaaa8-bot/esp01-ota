...
#define REQUIRE_WEB_LOGIN_FOR_GITHUB_UPDATE 1 // Manual GitHub update button is protected by WEB_OTA_USER / WEB_OTA_PASS.
...
const char* WEB_OTA_USER = "admin";    // Used only if WEB_OTA_PASS is not empty.
const char* WEB_OTA_PASS = "1977";     // Web OTA password. Change it before field use.

// Web users / permissions (V1.2.9)
// Admin: full control. Limited users: Control what you wish, Output 3 Smart, City Weather, Network Setup only.
// V1.2.9 adds an Admin-only User Management panel: enable/disable limited users and change passwords from the web.
#define WEB_ROLE_ADMIN   0
#define WEB_ROLE_LIMITED 1
#define WEB_USER_COUNT   6
#define WEB_PASS_MAX_LEN 16
struct WebUserAccount { const char* user; char pass[WEB_PASS_MAX_LEN + 1]; byte role; byte enabled; };
WebUserAccount WEB_USERS[WEB_USER_COUNT] = {
  {WEB_OTA_USER, "1977", WEB_ROLE_ADMIN, 1},
  {"user1", "1234", WEB_ROLE_LIMITED, 1},
  {"user2", "1234", WEB_ROLE_LIMITED, 1},
  {"user3", "1234", WEB_ROLE_LIMITED, 1},
  {"user4", "1234", WEB_ROLE_LIMITED, 1},
  {"user5", "1234", WEB_ROLE_LIMITED, 1}
};
byte currentWebRole = WEB_ROLE_LIMITED;
...
const unsigned long WIFI_CONNECT_TIMEOUT    = 8000UL;  // 8 seconds per attempt
...
const unsigned long THINGSPEAK_MIN_WRITE_MS = 16000UL; // ThingSpeak free-channel write spacing safety
...
#define EEPROM_SMART_MAGIC        0xE5  // changed in V1.2.6: timer is runtime-only, not stored in EEPROM

// Web User Management EEPROM Configuration (V1.2.9)
// Stored above the old 512-byte map to avoid moving existing settings.
#define EEPROM_USER_MAGIC_ADDR   520
#define EEPROM_USER_DATA_ADDR    524
#define EEPROM_USER_MAGIC        0xF1
...
const char* LOCAL_TZ_RULE = "EET-2EEST,M4.5.5/0,M10.5.5/0"; // Egypt: UTC+2, DST UTC+3 from last Friday of April to last Friday of October.
const long  LOCAL_GMT_OFFSET_SEC = 2 * 3600L; // Fallback only if TZ rule is not used.
...
struct SmartOutputConfig {
  ...
  byte repeatWeekly;
  byte onceDone;
  byte dayEnabled[7];      // tm_wday: 0=Sun ... 6=Sat
  uint16_t dayStartMin[7]; // minutes from 00:00 for every day
  uint16_t dayEndMin[7];
  int16_t tempOnX10;
  int16_t tempOffX10;
  int16_t humOnX10;
  int16_t humOffX10;
...
String wifiModeText() {
  return wifiManagerMode == WIFI_MODE_STA_FALLBACK ? "1 - STA only + fallback AP" : "0 - AP + STA always";
}
...
int webUserDataAddr(byte userIndex) {
  // For every user: 1 enabled byte + WEB_PASS_MAX_LEN+1 bytes password.
  return EEPROM_USER_DATA_ADDR + userIndex * (WEB_PASS_MAX_LEN + 2);
}

bool isSafePassword(const String& p) {
  if (p.length() < 1 || p.length() > WEB_PASS_MAX_LEN) return false;
  for (uint16_t i = 0; i < p.length(); i++) {
    char c = p[i];
    if (c < 33 || c > 126) return false; // no spaces/control chars; keeps Basic Auth simple.
  }
  return true;
}
...
void saveWebUsersConfig() {
  ...
  for (byte i = 0; i < WEB_USER_COUNT; i++) {
    ...
    // Admin is always enabled; limited users can be enabled/disabled.
    byte en = (i == 0) ? 1 : (WEB_USERS[i].enabled ? 1 : 0);
    EEPROM.write(addr, en);
    for (byte j = 0; j <= WEB_PASS_MAX_LEN; j++) {
      EEPROM.write(addr + 1 + j, WEB_USERS[i].pass[j]);
    }
  }
  EEPROM.commit();
}

bool loadedPassLooksValid(char* p) {
  if (p[0] == 0 || p[0] == (char)0xFF) return false;
  for (byte i = 0; i < WEB_PASS_MAX_LEN && p[i]; i++) {
    if (p[i] < 33 || p[i] > 126) return false;
  }
  return true;
}
...
void loadWebUsersConfig() {
  ...
  for (byte i = 0; i < WEB_USER_COUNT; i++) {
    int addr = webUserDataAddr(i);
    byte en = EEPROM.read(addr);
    char p[WEB_PASS_MAX_LEN + 1];
    memset(p, 0, sizeof(p));
    for (byte j = 0; j <= WEB_PASS_MAX_LEN; j++) p[j] = (char)EEPROM.read(addr + 1 + j);
    if (i == 0) WEB_USERS[i].enabled = 1;
    else WEB_USERS[i].enabled = (en == 1) ? 1 : 0;
    if (loadedPassLooksValid(p)) {
      memset(WEB_USERS[i].pass, 0, sizeof(WEB_USERS[i].pass));
      strncpy(WEB_USERS[i].pass, p, WEB_PASS_MAX_LEN);
    }
  }
}
...
String userManagementCardHtml() {
  String html;
  html.reserve(3300);
  html += F("<div class='card'><h1>User Management</h1><div class='sub'>Admin only. Enable/disable limited users and change passwords. User names are fixed to save ESP01 memory.</div>");
  html += F("<form method='POST' action='/saveusers'>");
  for (byte i = 0; i < WEB_USER_COUNT; i++) {
    html += F("<div class='smsec'><b>"); html += WEB_USERS[i].user; html += F("</b> <span class='pill'>"); html += (WEB_USERS[i].role == WEB_ROLE_ADMIN ? F("Admin") : F("Limited")); html += F("</span>");
    if (i > 0) {
      ...
      html += F("<div class='sub'>Admin account is always enabled.</div>");
    }
  ...
  html += F("<button class='btn' type='submit'>Save Users</button></form><div class='sub warn'>Allowed password length: 1-"); html += String(WEB_PASS_MAX_LEN); html += F(" printable characters, no spaces.</div></div>");
  return html;
}

void sendUsersPage() {
  if (!webOtaAuthOk()) return;
...
void handleSaveUsers() {
  if (!webOtaAuthOk()) return;
  for (byte i = 0; i < WEB_USER_COUNT; i++) {
    if (i > 0) WEB_USERS[i].enabled = webServer.hasArg(String("en") + i) ? 1 : 0;
    String newPass = webServer.arg(String("pw") + i);
    newPass.trim();
    if (newPass.length() > 0) setWebUserPassword(i, newPass);
  }
  saveWebUsersConfig();
  String html = htmlHeader("Users Saved");
  html += F("<div class='card'><h1>Users Saved</h1><div class='sub'>User permissions/passwords saved. If you changed your current password, the browser may ask you to login again.</div><a class='btn btn2' href='/users'>Back to Users</a><a class='btn btn2' href='/'>Home</a></div>");
  html += htmlFooter();
  webServer.send(200, "text/html", html);
}
...
bool webAuthCheck(bool adminOnly) {
  ...
  for (byte i = 0; i < WEB_USER_COUNT; i++) {
    ...
    if (webServer.authenticate(WEB_USERS[i].user, WEB_USERS[i].pass)) {
      currentWebRole = WEB_USERS[i].role;
...
bool webAnyAuthOk() {
  return webAuthCheck(false);
}

bool webOtaAuthOk() {
  return webAuthCheck(true);
}

bool webIsAdmin() {
  return currentWebRole == WEB_ROLE_ADMIN;
}
...
bool smartScheduleActiveNow() {
  ...
  // Same-day window, for example Mon 08:00 -> 12:00.
  if (smartCfg.dayEnabled[today]) {
    uint16_t st = smartCfg.dayStartMin[today];
    uint16_t en = smartCfg.dayEndMin[today];
  ...
  if (smartCfg.dayEnabled[prevDay]) {
    uint16_t st = smartCfg.dayStartMin[prevDay];
    uint16_t en = smartCfg.dayEndMin[prevDay];
...
void setSmartDefaults() {
  ...
  for (byte i = 0; i < 7; i++) {
    ...
    smartCfg.dayStartMin[i] = 18 * 60;
    smartCfg.dayEndMin[i] = 23 * 60;
  }
...
bool smartConfigLooksValid() {
  ...
  for (byte i = 0; i < 7; i++) {
    ...
    if (smartCfg.dayStartMin[i] > 1439 || smartCfg.dayEndMin[i] > 1439) return false;
  }
  return true;
}
...
String cityWeatherCardHtml() {
  String html;
  html.reserve(1400);
  html += F("<div class='card'><h1>City Weather</h1><div class='sub'>External weather by selected city. This does not use extra ESP01 pins.</div>");
...
String dayScheduleRow(byte d) {
  String x;
  x += F("<div class='dayrow'><label class='small'><input type='checkbox' name='en");
  x += String(d);
  x += F("' ");
  if (smartCfg.dayEnabled[d]) x += F("checked");
  x += F("> ");
  x += dayName(d);
  x += F("</label><input type='time' name='st");
  x += String(d);
  x += F("' value='");
  x += minuteToTime(smartCfg.dayStartMin[d]);
  x += F("'><input type='time' name='et");
  x += String(d);
  x += F("' value='");
  x += minuteToTime(smartCfg.dayEndMin[d]);
  x += F("'></div>");
  return x;
}
...
String smartOutputCardHtml() {
  ...
  html += F("<div class='sub'>Enable each day and set its own ON/OFF time. Overnight periods like 22:00 to 02:00 are supported.</div>");
  html += F("<div class='dayrow small'><b>Day</b><b>ON</b><b>OFF</b></div>");
  ...
  html += F("<div class='timergrid'><div class='titem'><label class='small'>Hours</label><div class='tctrl'><button class='tbtn' type='button' onclick=\"adjTimer('th',1)\">▲</button><input id='th' type='number' min='0' max='23' step='1' name='th' value='"); html += String(th); html += F("'><button class='tbtn' type='button' onclick=\"adjTimer('th',-1)\">▼</button></div></div>");
  html += F("<div class='titem'><label class='small'>Minutes</label><div class='tctrl'><button class='tbtn' type='button' onclick=\"adjTimer('tm',1)\">▲</button><input id='tm' type='number' min='0' max='59' step='1' name='tm' value='"); html += String(tm); html += F("'><button class='tbtn' type='button' onclick=\"adjTimer('tm',-1)\">▼</button></div></div>");
  html += F("<div class='titem'><label class='small'>Seconds</label><div class='tctrl'><button class='tbtn' type='button' onclick=\"adjTimer('ts',1)\">▲</button><input id='ts' type='number' min='0' max='59' step='1' name='ts' value='"); html += String(ts); html += F("'><button class='tbtn' type='button' onclick=\"adjTimer('ts',-1)\">▼</button></div></div></div>");
  ...
  html += F("<script>function adjTimer(id,d){var e=document.getElementById(id);if(!e||e.disabled)return;var mn=parseInt(e.min||0),mx=parseInt(e.max||59),v=parseInt(e.value||0)+d;if(v<mn)v=mn;if(v>mx)v=mx;e.value=v;var h=parseInt(document.getElementById('th').value||0),m=parseInt(document.getElementById('tm').value||0),s=parseInt(document.getElementById('ts').value||0);if(h==0&&m==0&&s==0)document.getElementById('ts').value=1;}function modeChanged(){var m=document.querySelector('input[name=mode]:checked').value;document.querySelectorAll('.smsec').forEach(function(s){var on=s.getAttribute('data-mode')==m;s.classList.toggle('disabled',!on);s.querySelectorAll('input,select,button').forEach(function(e){e.disabled=!on;});});}modeChanged();</script>");
  html += F("</div>");
  return html;
}
...
void handleSaveSmartOutput() {
  ...
  if (webServer.hasArg("st0") || webServer.hasArg("et0")) {
    for (byte i = 0; i < 7; i++) {
      smartCfg.dayEnabled[i] = webServer.hasArg(String("en") + i) ? 1 : 0;
      smartCfg.dayStartMin[i] = parseTimeToMinutes(webServer.arg(String("st") + i), smartCfg.dayStartMin[i]);
      smartCfg.dayEndMin[i] = parseTimeToMinutes(webServer.arg(String("et") + i), smartCfg.dayEndMin[i]);
    }
  }
...
String outputControlCardHtml() {
  ...
  html += F("<div class='msg' id='outmsg'>Ready. Status refreshes automatically every 5 seconds.</div></div>");
  return html;
}
...
void sendHomePage() {
  ...
  html += F("<div class='row'><span class='k'>Login Role</span><span class='v'>"); html += webIsAdmin() ? F("Admin") : F("Limited User"); html += F("</span></div>");
  html += F("<a class='btn btn2' href='/wifi'>WiFi / Network Setup</a></div><div class='card'>");
  ...
  html += F("</div>");
  html += outputControlCardHtml();
  html += smartOutputCardHtml();
  ...
  html += F("<div class='card'><a class='btn btn2' href='/restart'>Restart ESP01</a><div class='sub warn' style='margin-top:10px'>Login: "); html += WEB_OTA_USER; html += F(" / protected password</div></div>");
  html += F("<script>");
...
void handleSaveWifi() {
  ...
  html += F("<div class='card'><h1>WiFi Saved</h1><div class='sub'>Settings saved. ESP01 will restart and connect using the selected mode.</div></div>");
  html += htmlFooter();
  webServer.send(200, "text/html", html);
  delay(800);
  ESP.restart();
...
void handleForgetWifi() {
  ...
  html += F("<div class='card'><h1>WiFi Deleted</h1><div class='sub'>Saved network deleted. AP mode is enabled. Connect to the setup AP and open 192.168.4.1.</div></div>");
  html += htmlFooter();
  webServer.send(200, "text/html", html);
  delay(800);
  ESP.restart();
...
void sendCaptivePortalPage() {
  ...
  html += target;
  html += F("'><title>ESP01 Setup</title></head>");
  html += F("<body style='font-family:Arial;text-align:center;padding:30px'>");
  html += F("<h2>ESP01 Setup Portal</h2><p>Opening control page...</p><p><a href='");
  html += target;
  html += F("'>Open ESP01 Control Page</a></p>");
  html += F("<script>setTimeout(function(){location.href='");
  html += target;
  html += F("';},300);</script></body></html>");
  sendNoCacheHeaders();
  webServer.send(200, "text/html", html);
}

void sendCaptivePortalText() {
  sendNoCacheHeaders();
  webServer.send(200, "text/plain", "ESP01 captive portal");
}
...
void setupWebOTA() {
  ...
  webServer.on(WEB_OTA_PATH, HTTP_GET, sendHomePage);
  ...
  // V1.2.12 Strong Captive Portal probes for Android / iOS / Windows / Kindle.
  // Return 200 with a tiny HTML page instead of only 302; this triggers captive popup on more phones.
  webServer.on("/generate_204", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/gen_204", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/mobile/status.php", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/hotspot-detect.html", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/library/test/success.html", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/success.txt", HTTP_GET, sendCaptivePortalText);
  webServer.on("/ncsi.txt", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/connecttest.txt", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/redirect", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/canonical.html", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/fwlink", HTTP_GET, sendCaptivePortalPage);
  webServer.on("/kindle-wifi/wifiredirect.html", HTTP_GET, sendCaptivePortalPage);
  ...
  webServer.onNotFound([]() {
    // Strong captive catch-all: any unknown host/path returns a 200 portal page.
    // This improves automatic popup behavior compared with 404 or redirect-only responses.
...
void startWiFiAttempt() {
  ...
  if (wifiManagerMode == WIFI_MODE_AP_STA_ALWAYS || activeSsid[0] == '\0') {
  ...
  if (activeSsid[0] == '\0') {
    DBG_PRINTLN("No saved WiFi. AP setup mode only.");
    startConfigAP();
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (wifiManagerMode == WIFI_MODE_AP_STA_ALWAYS) startConfigAP();
    writeStatusLed(true);
    setupOTA();
    setupWebOTA();
  } else {
    DBG_PRINTLN("WiFi not connected. Fallback AP is available.");
    if (wifiManagerMode == WIFI_MODE_STA_FALLBACK) {
      WiFi.mode(WIFI_AP_STA);
      startConfigAP();
    }
    writeStatusLed(false);
...
void maintainWiFi() {
  if (WiFi.status() == WL_CONNECTED) {
    if (!printedOnlineThingSpeakMode) {
      DBG_PRINTLN("WiFi connected -> ThingSpeak control active");
      DBG_PRINTLN("Current states still remain backed up in EEPROM.");
  ...
  if (!printedOfflineEepromMode) {
    DBG_PRINTLN("WiFi disconnected -> running from EEPROM/last applied states");
...
bool uploadOutputStatesOnly(const char* reason) {
  if (!thingSpeakConfigured(true)) { DBG_PRINTLN("ThingSpeak write skipped: not configured"); markOutputThingSpeakUpdatePending(reason); return false; }
  if (WiFi.status() != WL_CONNECTED) {
    DBG_PRINTLN("Cannot upload manual output update: WiFi disconnected");
    markOutputThingSpeakUpdatePending(reason);
    return false;
  }

  unsigned long now = millis();
  if (lastThingSpeakWriteTime != 0 && now - lastThingSpeakWriteTime < THINGSPEAK_MIN_WRITE_MS) {
    DBG_PRINTLN("ThingSpeak write spacing not ready -> output update kept pending");
    markOutputThingSpeakUpdatePending(reason);
    return false;
  }

  DBG_PRINTLN("==== THINGSPEAK OUTPUT UPDATE ====");
  DBG_PRINT("Reason : ");
  DBG_PRINTLN(reason);
  DBG_PRINT("Output1: ");
  DBG_PRINTLN(currentOutput1State ? "ON" : "OFF");
  ...
  DBG_PRINT("ThingSpeak output update status: ");
  DBG_PRINTLN(status);
  ...
  if (status == 200) {
    ...
    DBG_PRINTLN("ThingSpeak output update OK -> pending flag cleared after confirmed success");
    DBG_PRINTLN("==================================");
    return true;
  }
...
bool readAndSendSensorData() {
  if (!thingSpeakConfigured(true)) { DBG_PRINTLN("Sensor upload skipped: ThingSpeak not configured"); return false; }
  if (WiFi.status() != WL_CONNECTED) {
    DBG_PRINTLN("Skip sensor upload: WiFi disconnected");
    return false;
  }
  ...
  if (status == 200) {
    ...
    DBG_PRINTLN("ThingSpeak upload OK");
    return true;
  }

  DBG_PRINT("ThingSpeak upload failed. Status: ");
  DBG_PRINTLN(status);
  return false;
}
